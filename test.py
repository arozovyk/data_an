...
def fit_garch(data):
    model = arch_model(data, vol='Garch', p=1, q=1, mean='Constant', dist='Normal')
...
def update(attr, old, new):
    ...
    p_arima.x_range.start = future_dates[0]
    p_arima.x_range.end = future_dates[-1]
    p_garch.x_range.start = future_dates[0]
    p_garch.x_range.end = future_dates[-1]
...
p_arima = figure(x_axis_type='datetime', width=800, height=400, title='ARIMA Forecast',
                 tools='pan,wheel_zoom,box_zoom,reset,save', x_range=p.x_range)
...
layout = column(row(crypto_select, date_range_slider), p, p_arima, p_garch)
