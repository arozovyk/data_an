...
p = figure(x_axis_type='datetime', width=900, height=400, title='Historical Crypto Prices', tools='pan,wheel_zoom,box_zoom,reset,save')
...
p.add_tools(hover)
p_volume = figure(x_axis_type='datetime', width=900, height=200, title='Total Traded Volume', tools='pan,wheel_zoom,box_zoom,reset,save')
...
p_arima = figure(x_axis_type='datetime', width=900, height=200, title='ARIMA Forecast', tools='pan,wheel_zoom,box_zoom,reset,save')
...
p_rsi = figure(x_axis_type='datetime', width=900, height=200, title='Relative Strength Index (RSI)', tools='pan,wheel_zoom,box_zoom,reset,save')
p_rsi.line(x='Date', y='RSI', source=rsi_source, color='green', legend_label='RSI')
p_rsi.yaxis.axis_label = 'RSI'
p_rsi.y_range.start = 0
p_rsi.y_range.end = 100
hover_rsi = HoverTool(tooltips=[('Date', '@Date{%F}'), ('RSI', '@RSI{0.00}')], formatters={'@Date': 'datetime'})
p_rsi.add_tools(hover_rsi)

layout = row(column(crypto_select, date_range_slider), column(p, p_volume), column(p_arima, p_rsi))
